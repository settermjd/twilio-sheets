...
final class OrderDeskService
{
    ...
    public function __construct(private string $store_id, private string $api_key)
    {
    ...
    public function post(string $url, ?array $post = null)
    {
        return $this->go("POST", $url, $post);
    }

    public function put(string $url, ?array $post = null)
    {
        return $this->go("PUT", $url, $post);
    }

    public function delete(string $url, ?array $post = null)
    {
        return $this->go("DELETE", $url, $post);
    }

    public function go(string $method, string $url, ?array $post = null)
    {
        if (! is_array($post)) {
            $post = null;
        }
    ...
    public function validatePostedJson()
    {
        ...
        if (
            ! isset($_SERVER[ 'HTTP_X_ORDER_DESK_STORE_ID' ])
                || $_SERVER[ 'HTTP_X_ORDER_DESK_STORE_ID' ] != $this->store_id
        ) {
            header(':', true, 403);
        ...
        if (
            ! isset($_SERVER[ 'HTTP_X_ORDER_DESK_HASH' ])
                || hash_hmac('sha256', rawurldecode($_POST[ 'order' ]), $this->api_key) != $_SERVER[ 'HTTP_X_ORDER_DESK_HASH' ]
        ) {
            header(':', true, 403);
            die('Unauthorized Request');
        }
